...
export const SEOHeroEffect = ({
  ...
  return (
    <div className={cn("sticky top-80", className)}>
      ...
      {/* URL Input Form - replacing the center button */}
      <div className="w-full h-[890px] -top-60 md:-top-40 flex items-center justify-center absolute">
        <form
          onSubmit={handleSubmit}
          className="z-30 md:mt-24 mt-8 w-full max-w-[560px] mx-auto px-4"
        >
          <div
            className={cn(
              "relative flex items-center rounded-full transition-all duration-300",
              "bg-white/[0.08] backdrop-blur-xl border",
              isFocused
                ? "border-white/30 shadow-[0_0_30px_rgba(7,110,255,0.15)]"
                : "border-white/[0.12] shadow-[0_4px_24px_rgba(0,0,0,0.2)]",
              "hover:border-white/20"
            )}
          >
            <Search
              size={18}
              className="absolute left-4 md:left-5 text-neutral-500 pointer-events-none"
            />
            <input
              type="text"
              value={url}
          ...
          <div className="flex items-center justify-center gap-2 mt-4 flex-wrap">
            ...
            {["stripe.com", "linear.app", "vercel.com"].map((example) => (
              <button
                ...
                className="text-[11px] md:text-xs text-neutral-500 hover:text-white px-2.5 py-1 rounded-full border border-white/[0.06] hover:border-white/20 transition-all duration-200 hover:bg-white/[0.04]"
              >
                {example}
              </button>
            ))}
